...
namespace MinecraftModGenerator
{
    internal class ModelCreator : Creator
    {
        ...
        public void CreateTrapdoor(string trapdoorName)
        {
            string texTrapdoorName = FuncBlockTex(trapdoorName);

            var variants = new Dictionary<string, VariantModel>();
            var dirDict = DirectionsOrdered();
            ...
            foreach (string dir in dirDict.Keys) {
                ...
                for (int i = 0; i < 2; i++) {
                    bool bottom = (i == 0);
                    string bottomOrTop = (bottom ? "bottom" : "top");
                    string adder = $"facing={dir},half={ bottomOrTop },";

                    variants.Add($"{adder}open=false", new($"{texTrapdoorName}_{bottomOrTop}", false, y: dirAngle));
                    variants.Add($"{adder}open=true", new($"{texTrapdoorName}_open", false, x: bottom ? default : 180, y: CleanAngle(bottom ? dirAngle : dirAngle + 180)));
                }
            }

            var trapdoorModelDict = ModelsRoot.DictSingle(texTrapdoorName, "texture");
            FileBlockstate(trapdoorName, variants);
            FileModel(trapdoorName, new() {
                { trapdoorName + "_bottom", ModelsRoot.Parented(this, "template_orientable_trapdoor_bottom", trapdoorModelDict, false) },
                { trapdoorName + "_open", ModelsRoot.Parented(this, "template_orientable_trapdoor_open", trapdoorModelDict, false) },
                { trapdoorName + "_top", ModelsRoot.Parented(this, "template_orientable_trapdoor_top", trapdoorModelDict, false) }
            }, itemModel: new ItemModel(texTrapdoorName + "_bottom"));
        }
        ...
        public void CreatePressurePlate(string pressurePlateName, string texBlockName)
        {
            string texPressurePlateName = FuncBlockTex(pressurePlateName);

            var pressurePlateModelDict = ModelsRoot.DictSingle(texBlockName, "texture");
            FileBlockstate(pressurePlateName, new Dictionary<string, VariantModel>() {
                { "powered=false", new(texPressurePlateName) },
                { "powered=true", new(texPressurePlateName + "_down") }
            });
            FileModel(pressurePlateName, new() {
                { pressurePlateName, ModelsRoot.Parented(this, "pressure_plate_up", pressurePlateModelDict, false) },
        ...
        public void FamilyWoodType(string woodTypeName, string saplingName = null)
        {
            saplingName = (string.IsNullOrEmpty(saplingName)) ? woodTypeName + "_sapling" : saplingName;
            ...
            CreateWood($"stripped_{woodTypeName}_wood");
            ...
            CreateTrapdoor($"{woodTypeName}_trapdoor");
            CreateButton($"{woodTypeName}_button", texPlank);
            CreatePressurePlate($"{woodTypeName}_pressure_plate", texPlank);
